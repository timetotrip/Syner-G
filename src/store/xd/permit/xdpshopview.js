  ...
  constructor () {
      ...
      setCProducts ({ commit, state, rootGetters }, sid) {
        ...
        for (const bid in state.cShop.brands) {
          ...
          dXdsCShop.push(plistRef.withConverter(cfBrands.ConvCProduct).onSnapshot((snap) => {
            ...
            this.dispatch('xd/permit/xdpshopview/setProductsCreatives', plist, { root: true })
          }))
        }
    ...
    this.getters = {
      ...
      cProCreativeIds: (state, getters, rootState, rootGetters) => (pid) => {
        const creatives = rootGetters['xd/permit/xdpshopview/cProCreatives'](pid)
        if (creatives === {}) {
          return []
        }
      ...
      cProTopCrePath: (state, getters, rootState, rootGetters) => (pid) => {
        ...
        const pCreativeIds = rootGetters['xd/permit/xdpshopview/cProCreativeIds'](pid)
        if (pCreativeIds.length > 0) {
          return rootGetters['xd/permit/xdpshopview/cCrePath'](pCreativeIds[0])
        }
        return pathLocading
      }
    }
